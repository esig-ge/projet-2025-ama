<?php
// /site/pages/facture_pdf.php
declare(strict_types=1);
session_start();

// Projet racine: .../Projet_sur_Mandat
$ROOT = dirname(__DIR__, 2);

// Charge Dompdf (version repo GitHub sans Composer)
$dompdfAutoloader = $ROOT . '/app/libs/dompdf/Autoloader.php';
if (!is_file($dompdfAutoloader)) {
    http_response_code(500);
    exit("Autoloader Dompdf introuvable: " . htmlspecialchars($dompdfAutoloader));
}
require_once $dompdfAutoloader;
Dompdf\Autoloader::register();

// Exemple minimal d’usage
use Dompdf\Dompdf;

$dompdf = new Dompdf([
    'isRemoteEnabled' => true, // si tu charges des images par URL
    'chroot' => $ROOT,         // optionnel: limite les chemins
]);

$html = '<h1>Facture DK Bloom</h1><p>Commande #12345</p>';
$dompdf->loadHtml($html);
$dompdf->setPaper('A4', 'portrait');
$dompdf->render();
$dompdf->stream('facture_12345.pdf', ['Attachment' => true]);

/* ===== Gardes ===== */
if (empty($_SESSION['per_id'])) { http_response_code(403); exit('Veuillez vous connecter.'); }
$perId = (int)$_SESSION['per_id'];

/* ===== Bases de chemins ===== */
$dir  = rtrim(dirname($_SERVER['PHP_SELF'] ?? $_SERVER['SCRIPT_NAME']), '/\\');
$BASE = ($dir === '' || $dir === '.') ? '/' : $dir . '/';
$SITE_BASE = preg_replace('#pages/$#', '', $BASE); // ex: /site/

/** @var PDO $pdo */
$pdo = require __DIR__ . '/../database/config/connexionBDD.php';
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

/* ===== Helpers ===== */
function h($s){ return htmlspecialchars((string)$s, ENT_QUOTES,'UTF-8'); }
function fetchOne(PDO $pdo,string $sql,array $p){ $st=$pdo->prepare($sql); $st->execute($p); return $st->fetch(PDO::FETCH_ASSOC) ?: null; }
function columnExists(PDO $pdo, string $table, string $column): bool {
    $st = $pdo->prepare("SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
                         WHERE TABLE_SCHEMA = DATABASE()
                           AND TABLE_NAME = :t
                           AND COLUMN_NAME = :c LIMIT 1");
    $st->execute([':t'=>$table, ':c'=>$column]);
    return (bool)$st->fetchColumn();
}
$fmt = fn($n) => number_format((float)$n, 2, '.', ' ') . ' CHF';

function img_to_data_uri(string $absPath): ?string {
    if (!is_file($absPath)) return null;
    $ext = strtolower((string)pathinfo($absPath, PATHINFO_EXTENSION));
    $mime = in_array($ext, ['png','jpg','jpeg','gif','webp','svg']) ? "image/$ext" : 'application/octet-stream';
    $data = base64_encode((string)file_get_contents($absPath));
    return "data:$mime;base64,$data";
}

/* ===== Param ===== */
$comId = isset($_GET['com_id']) && ctype_digit((string)$_GET['com_id']) ? (int)$_GET['com_id'] : 0;
if ($comId <= 0) { http_response_code(400); exit('Commande invalide.'); }

/* ===== 1) En-tête commande (ownership) ===== */
$sqlHead = "
  SELECT c.COM_ID, c.PER_ID, c.COM_DATE, c.COM_STATUT, c.LIV_ID, c.PAI_ID
  FROM COMMANDE c
  WHERE c.COM_ID = :cid AND c.PER_ID = :per
  LIMIT 1";
$head = fetchOne($pdo, $sqlHead, [':cid'=>$comId, ':per'=>$perId]);
if (!$head) { http_response_code(404); exit('Commande introuvable.'); }

/* Paiement & livraison */
$paiement = fetchOne($pdo, "
  SELECT PAI_ID, PAI_MODE, PAI_STATUT, PAI_MONTANT
  FROM PAIEMENT WHERE PAI_ID = :pid
", [':pid' => (int)($head['PAI_ID'] ?? 0)]) ?: [];

$livraison = fetchOne($pdo, "
  SELECT LIV_ID, LIV_STATUT
  FROM LIVRAISON WHERE LIV_ID = :lid
", [':lid' => (int)($head['LIV_ID'] ?? 0)]) ?: [];

/* ===== 2) Adresse de livraison (détection schéma) ===== */
$adresse = null;
if (columnExists($pdo, 'COMMANDE', 'ADR_ID_LIVRAISON')) {
    $adresse = fetchOne($pdo, "
      SELECT a.ADR_RUE, a.ADR_NUMERO, a.ADR_NPA, a.ADR_VILLE, a.ADR_PAYS
      FROM COMMANDE c
      JOIN ADRESSE a ON a.ADR_ID = c.ADR_ID_LIVRAISON
      WHERE c.COM_ID = :cid
      LIMIT 1
    ", [':cid'=>$comId]);
}
if (!$adresse && columnExists($pdo, 'COMMANDE', 'ADR_ID')) {
    $adresse = fetchOne($pdo, "
      SELECT a.ADR_RUE, a.ADR_NUMERO, a.ADR_NPA, a.ADR_VILLE, a.ADR_PAYS
      FROM COMMANDE c
      JOIN ADRESSE a ON a.ADR_ID = c.ADR_ID
      WHERE c.COM_ID = :cid
      LIMIT 1
    ", [':cid'=>$comId]);
}
if (!$adresse && !empty($head['LIV_ID']) && columnExists($pdo, 'LIVRAISON','ADR_ID')) {
    $adresse = fetchOne($pdo, "
      SELECT a.ADR_RUE, a.ADR_NUMERO, a.ADR_NPA, a.ADR_VILLE, a.ADR_PAYS
      FROM LIVRAISON l
      JOIN ADRESSE a ON a.ADR_ID = l.ADR_ID
      WHERE l.LIV_ID = :lid
      LIMIT 1
    ", [':lid'=>(int)$head['LIV_ID']]);
}

/* ===== 3) Lignes produits ===== */
$st = $pdo->prepare("
  SELECT cp.PRO_ID, p.PRO_NOM, p.PRO_PRIX AS prix_u,
         COALESCE(cp.CP_QTE_COMMANDEE, 1) AS qte
  FROM COMMANDE_PRODUIT cp
  JOIN PRODUIT p ON p.PRO_ID = cp.PRO_ID
  WHERE cp.COM_ID = :cid
  ORDER BY p.PRO_NOM");
$st->execute([':cid'=>$comId]);
$items = $st->fetchAll(PDO::FETCH_ASSOC);

/* ===== 4) Suppléments (optionnel) ===== */
$supps = [];
try {
    $st = $pdo->prepare("
      SELECT s.SUP_NOM, s.SUP_PRIX_UNITAIRE AS prix_u, COALESCE(cs.CS_QTE_COMMANDEE,1) AS qte
      FROM COMMANDE_SUPP cs
      JOIN SUPPLEMENT s ON s.SUP_ID = cs.SUP_ID
      WHERE cs.COM_ID = :cid
      ORDER BY s.SUP_NOM");
    $st->execute([':cid'=>$comId]);
    $supps = $st->fetchAll(PDO::FETCH_ASSOC);
} catch (Throwable $e){}

/* ===== 5) Emballages (optionnel) ===== */
$embs = [];
try {
    $st = $pdo->prepare("
      SELECT e.EMB_NOM, 0 AS prix_u, 1 AS qte
      FROM COMMANDE_EMBALLAGE ce
      JOIN EMBALLAGE e ON e.EMB_ID = ce.EMB_ID
      WHERE ce.COM_ID = :cid
      ORDER BY e.EMB_NOM");
    $st->execute([':cid'=>$comId]);
    $embs = $st->fetchAll(PDO::FETCH_ASSOC);
} catch (Throwable $e){}

/* ===== 6) Totaux ===== */
$subtotal = 0.0; foreach ($items as $it) { $subtotal += (float)$it['prix_u'] * (int)$it['qte']; }
$suppTotal= 0.0; foreach ($supps as $sp) { $suppTotal+= (float)$sp['prix_u'] * (int)$sp['qte']; }
$embTotal = 0.0; foreach ($embs as $em) { $embTotal += (float)$em['prix_u'] * (int)$em['qte']; }
$grandTotal = $subtotal + $suppTotal + $embTotal;
$paidAmount = isset($paiement['PAI_MONTANT']) ? ((float)$paiement['PAI_MONTANT']/100.0) : null;

/* ===== Assets (logo) en data URI ===== */
$logoAbs = __DIR__ . '/../img/logo.png'; // ajuste si ton logo est ailleurs
$logoUri = img_to_data_uri($logoAbs);

/* ===== HTML facture ===== */
$when = date('d.m.Y', strtotime((string)$head['COM_DATE']));
$statut = (string)$head['COM_STATUT'];
$addrHtml = $adresse
    ? h($adresse['ADR_RUE'].' '.$adresse['ADR_NUMERO']).'<br>'.h($adresse['ADR_NPA'].' '.$adresse['ADR_VILLE']).'<br>'.h($adresse['ADR_PAYS'])
    : '<span style="color:#888">Aucune adresse</span>';

$rowsHtml = '';
foreach ($items as $it) {
    $line = (float)$it['prix_u'] * (int)$it['qte'];
    $rowsHtml .= '<tr>
        <td>'.h($it['PRO_NOM']).'</td>
        <td class="r">'.$fmt((float)$it['prix_u']).'</td>
        <td class="r">'.(int)$it['qte'].'</td>
        <td class="r">'.$fmt($line).'</td>
    </tr>';
}
if ($supps) {
    $rowsHtml .= '<tr class="section"><td colspan="4"><b>Suppléments</b></td></tr>';
    foreach ($supps as $sp) {
        $line = (float)$sp['prix_u'] * (int)$sp['qte'];
        $rowsHtml .= '<tr>
            <td>'.h($sp['SUP_NOM']).'</td>
            <td class="r">'.$fmt((float)$sp['prix_u']).'</td>
            <td class="r">'.(int)$sp['qte'].'</td>
            <td class="r">'.$fmt($line).'</td>
        </tr>';
    }
}
if ($embs) {
    $rowsHtml .= '<tr class="section"><td colspan="4"><b>Emballages</b></td></tr>';
    foreach ($embs as $em) {
        $line = (float)$em['prix_u'] * (int)$em['qte'];
        $rowsHtml .= '<tr>
            <td>'.h($em['EMB_NOM']).'</td>
            <td class="r">'.$fmt((float)$em['prix_u']).'</td>
            <td class="r">'.(int)$em['qte'].'</td>
            <td class="r">'.$fmt($line).'</td>
        </tr>';
    }
}


$html = '
<!doctype html>
<html lang="fr">
<meta charset="utf-8">
<style>
@page { margin: 30px 28px; }
body{ font-family: DejaVu Sans, Arial, sans-serif; color:#1b1b1b; }
.header{ display:flex; align-items:center; justify-content:space-between; margin-bottom:14px; }
.brand{ color:#5C0012; }
.logo{ height:46px }
.h-sep{ height:2px; background:#5C0012; opacity:.12; margin:10px 0 16px; }
.small{ color:#666; font-size:12px }
h1{ font-size:20px; margin:6px 0 2px; color:#5C0012; }
.meta{ display:flex; gap:22px; margin:6px 0 16px; }
.meta .box{ background:#f7f1f2; border-radius:10px; padding:10px 12px; }
.meta .k{ font-size:11px; color:#6b6b6b; display:block; }
.addr{ line-height:1.4 }
table{ width:100%; border-collapse:collapse; font-size:13px }
th,td{ padding:8px 6px; border-bottom:1px solid #eee }
th{ text-align:left; color:#444 }
.r{ text-align:right }
.section td{ background:#faf5f6; font-size:12px; color:#5C0012; }
.totals{ margin-top:10px; }
.trow{ display:flex; justify-content:space-between; padding:4px 0; }
.grand{ border-top:1px solid #ddd; margin-top:6px; padding-top:8px; font-weight:700; }
.badge{ display:inline-block; background:#f1e7e9; color:#8A1B2E; padding:4px 8px; border-radius:999px; font-size:12px }
.footer{ margin-top:18px; font-size:11px; color:#666 }
</style>

<body>
  <div class="header">
    <div>
      <div class="brand"><strong>DK Bloom</strong></div>
      <div class="small">Bouquets & cadeaux — Genève</div>
    </div>'.
    ($logoUri ? '<img class="logo" src="'.$logoUri.'" alt="DK Bloom">' : '')
    .'</div>

  <div class="h-sep"></div>

  <h1>Facture #'.(int)$head['COM_ID'].'</h1>
  <div class="meta">
    <div class="box"><span class="k">Date</span>'.$when.'</div>
    <div class="box"><span class="k">Statut commande</span><span class="badge">'.h($statut).'</span></div>
    <div class="box"><span class="k">Paiement</span>'.h((string)($paiement['PAI_MODE'] ?? '—')).' — '.h((string)($paiement['PAI_STATUT'] ?? '—')).'</div>
  </div>

  <div class="addr">
    <strong>Adresse de livraison</strong><br>'.$addrHtml.'
  </div>

  <h2 style="font-size:16px;margin:14px 0 6px;color:#8A1B2E">Détail</h2>
  <table>
    <thead><tr><th>Article</th><th class="r">PU</th><th class="r">Qté</th><th class="r">Total</th></tr></thead>
    <tbody>'.$rowsHtml.'</tbody>
  </table>

  <div class="totals">
    <div class="trow"><span>Sous-total produits</span><strong>'.$fmt($subtotal).'</strong></div>'.
    ($suppTotal>0 ? '<div class="trow"><span>Suppléments</span><strong>'.$fmt($suppTotal).'</strong></div>' : '').
    ($embTotal>0 ? '<div class="trow"><span>Emballages</span><strong>'.$fmt($embTotal).'</strong></div>' : '').
    '<div class="trow grand"><span>Total</span><strong>'.$fmt($grandTotal).'</strong></div>'.
    ($paidAmount !== null ? '<div class="trow"><span>Montant payé</span><strong>'.$fmt($paidAmount).'</strong></div>' : '')
    .'</div>

  <div class="footer">
    Prix affichés TTC. Pour la Suisse, la TVA est incluse le cas échéant.
    En cas de question, contactez-nous depuis la page Contact.
  </div>
</body>
</html>';

/* ===== Génération PDF (Dompdf) ===== */
/* Autoloader robuste */
$ROOT = dirname(__DIR__, 2); // depuis /site/pages -> racine projet

$candidates = [
    $ROOT . '/../../../../app/libs/dompdf/Autoloader.php', // emplacement normal (clone/ZIP récents)
    $ROOT . '/app/libs/dompdf/Autoloader.php',      // au cas où
];
$found = null;
foreach ($candidates as $p) { if (is_file($p)) { $found = $p; break; } }
if (!$found) { http_response_code(500); die('Autoloader Dompdf introuvable: '.implode(' | ', $candidates)); }

require_once $found;
/* IMPORTANT : prefixe \ pour appeler depuis l'espace global */
\Dompdf\Autoloader::register();

/* Namespaces utilisés */
use Dompdf\Dompdf;
use Dompdf\Options;

$options = new Options();
$options->set('isRemoteEnabled', true);
$options->set('isHtml5ParserEnabled', true);
$options->set('defaultPaperSize', 'a4');

// (optionnel) répertoire temporaire si nécessaire
// @mkdir($ROOT . '/storage/dompdf_tmp', 0777, true);
// $options->set('tempDir', $ROOT . '/storage/dompdf_tmp');

$dompdf = new Dompdf($options);
$dompdf->loadHtml($html, 'UTF-8');
$dompdf->setPaper('A4', 'portrait');
$dompdf->render();

/* Nom de fichier propre */
$fname = 'Facture_DK_Bloom_COM-'.$head['COM_ID'].'_'.date('Ymd').'.pdf';
$dompdf->stream($fname, ['Attachment' => true]); // true = téléchargement; false = aperçu inline
exit;
